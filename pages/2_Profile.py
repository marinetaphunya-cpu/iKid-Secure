...
if not patient_id:
    ...
    if st.button("⬅️ กลับไปหน้ารายชื่อ", key="back_no_patient"):
...
with st.expander("📝 ประวัติความรุนแรงเดิม (บันทึกเพิ่มเติม)", expanded=True):
    ...
    new_history = st.text_area(
        ...
        height=100,
        key=f"history_note_{patient_id}",
    )
    if st.button("บันทึกโน้ตประวัติ", key=f"save_history_{patient_id}"):
...
if not st.session_state.edit_mode:
    if not df.empty:
        ...
        st.dataframe(
            ...
            key=f"assess_view_{patient_id}",
        )
    else:
        st.write("ยังไม่มีประวัติการประเมินในระบบ")

    if st.button("✏️ แก้ไขประวัติการประเมิน", key=f"edit_toggle_{patient_id}"):
    ...
    edited_df = st.data_editor(
        ...
        key=f"assess_editor_{patient_id}",
    )

    col_b1, col_b2 = st.columns([1, 5])
    if col_b1.button("💾 บันทึกการประเมิน", key=f"save_edit_{patient_id}"):
        try:
    ...
    if col_b2.button("❌ ยกเลิก", key=f"cancel_edit_{patient_id}"):
        st.session_state.edit_mode = False
        st.rerun()

st.divider()
if st.button("🚀 ไปหน้าประเมิน", use_container_width=True, key=f"go_eval_{patient_id}"):
    st.switch_page("pages/3_Evaluation.py")

if st.button("🏠 กลับหน้าหลัก", use_container_width=True, key=f"go_dashboard_{patient_id}"):
    st.switch_page("pages/1_Dashboard.py")
